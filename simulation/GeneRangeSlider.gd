...
var _dragging_min: bool = false
var _dragging_max: bool = false
...
func _draw() -> void:
	...
	# Active range
	var min_x: float = _value_to_x(min_value)
	var max_x: float = _value_to_x(max_value)
...
func _gui_input(event: InputEvent) -> void:
	...
	if mb != null and mb.button_index == MOUSE_BUTTON_LEFT:
		if mb.pressed:
			var mouse_x: float = mb.position.x
			var min_x: float = _value_to_x(min_value)
			var max_x: float = _value_to_x(max_value)
	...
	if mm != null and (_dragging_min or _dragging_max):
		_update_drag(mm.position.x)

func _update_drag(mouse_x: float) -> void:
	var value: float = _x_to_value(mouse_x)
	if _dragging_min:
		var next_min: float = _snap(clamp(value, min_limit, max_value))
		if not is_equal_approx(next_min, min_value):
			min_value = next_min
			emit_signal("range_changed", min_value, max_value)
	elif _dragging_max:
		var next_max: float = _snap(clamp(value, min_value, max_limit))
...
func _value_to_x(value: float) -> float:
	...
	var t: float = (value - min_limit) / (max_limit - min_limit)
	return lerp(handle_radius, max(size.x - handle_radius, handle_radius + 1.0), t)

func _x_to_value(x: float) -> float:
	var left: float = handle_radius
	var right: float = max(left + 1.0, size.x - handle_radius)
	var t: float = inverse_lerp(left, right, clamp(x, left, right))
